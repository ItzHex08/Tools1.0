...
def main_log():
        ...
        file_log.close()
        code_log = (datetime.now() + timedelta(hours=9)).strftime('[%H:%M:%S]')+" Installing File requirements.txt ...\n"
        time.sleep(1)
        file_log.write(code_log)
        file_log.close()
        code_log = (datetime.now() + timedelta(hours=9)).strftime('[%H:%M:%S]')+" Installing Folder data ...\n"
        time.sleep(1)
        file_log.write(code_log)
        file_log.close()
        code_log = (datetime.now() + timedelta(hours=9)).strftime('[%H:%M:%S]')+" Installing Folder log ...\n"
        time.sleep(1)
        file_log.write(code_log)
        file_log.close()
def main_start():
